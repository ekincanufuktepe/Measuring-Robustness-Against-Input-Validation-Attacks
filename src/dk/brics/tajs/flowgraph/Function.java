...
public class Function {
	...
	private static String fileFuncName = "FunctionNames.txt";
	private static String fileParamNames = "ParamNames.txt";

	/**
	 * Returns the function name, or null if the function is anonymous.
	 * Create a "FunctionNames.txt" File that holds it's location and 
	 * function name.
	 */
	Map <Integer, String> funcName = new HashMap<>();	
	public String getName() {

		File fileFunc = new File("variableInfo\\"+fileFuncName);
		if(!funcName.containsKey(location.getLineNumber()) && !funcName.containsValue(name))
		{
			funcName.put(location.getLineNumber(), name);


			try {

				if(fileFunc.exists() == false)
				{
					System.out.println("NEW");
					fileFunc.createNewFile();
				}

				PrintWriter outFunc = new PrintWriter( new FileWriter(fileFunc, true));

				//System.out.println("Line: "+location.getLineNumber()+" Function Name: "+name);
				outFunc.append(+location.getLineNumber()+" "+name+"\n");
				outFunc.close();	
			} catch (IOException e) {
	 ...
	 * Creates a "ParamNames.txt" file that holds Parameter Name list with 
	 * with their locations.
	 */
	Map <Integer, List<String>> paramName = new HashMap <>();
	public List<String> getParameterNames() {
		//		System.out.println("Line: "+location.getLineNumber()+" Parameter Names: "+parameter_names);
		//		Map <Integer, String> paramName = new HashMap <Integer, String>();
		//		paramName.put(location.getLineNumber(), parameter_names);
		File fileParam = new File("variableInfo\\"+fileParamNames);
		if(!paramName.containsKey(location.getLineNumber()) && !funcName.containsValue(parameter_names))
		{
			paramName.put(location.getLineNumber(), parameter_names);

			try {

				if(fileParam.exists() == false)
				{
					System.out.println("NEW");
					fileParam.createNewFile();
				}

				PrintWriter outFunc = new PrintWriter( new FileWriter(fileParam, true));	
				//System.out.println("Line: "+location.getLineNumber()+" Function Name: "+name);
				outFunc.append(location.getLineNumber()+" "+parameter_names+"\n");
				outFunc.close();
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
	 ...
	 */
	Map<Integer, Set<String>> declVarNames = new HashMap<>();
	public Set<String> getVariableNames() {
		File fileDecl = new File("variableInfo\\VariableNamesInFunc.txt");
		if(!declVarNames.containsKey(location.getLineNumber()) && !declVarNames.containsValue(java.util.Collections.unmodifiableSet(variable_names)))
		{
			try {
				if(fileDecl.exists() == false)
				{
					System.out.println("Created New File");
					fileDecl.createNewFile();
				}

				declVarNames.put(location.getLineNumber(), java.util.Collections.unmodifiableSet(variable_names));
				PrintWriter outFunc = new PrintWriter( new FileWriter(fileDecl, true));
				System.out.println(location.getLineNumber()+" - "+variable_names+"\n");
				outFunc.append(location.getLineNumber()+" "+java.util.Collections.unmodifiableSet(variable_names)+"\n");
				outFunc.close();	
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
		...
		//		System.out.println("Line: "+location.getLineNumber()+" Variables Declared in function: "+java.util.Collections.unmodifiableSet(variable_names));
	...
	public String toString() {
		...
		try {
			if(!fp.containsKey(location.getLineNumber()) && !fp.containsValue(b.toString()))
			{
				fp.put(location.getLineNumber(), b.toString());
				File vif = new File("files\\FunctionAndParams.txt");
				//BufferedWriter bw = new BufferedWriter(vif);
				PrintWriter bw = new PrintWriter( new FileWriter(vif, true));
				bw.append(location.getLineNumber()+" "+b.toString()+"\n");
				bw.close();
			}
	...
	public void toDot(PrintWriter pw, boolean standalone, boolean main) {
		...
		pw.println("//#$~ "+tmpBlock.getFunction().getOrdinaryExit().getSourceLocation().getLineNumber());
		pw.println("}");
		if (standalone) {
			pw.flush();
		}
