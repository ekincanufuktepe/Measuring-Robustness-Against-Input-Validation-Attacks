...
public class RhinoASTToFlowgraph { // TODO: improve javadoc
	...
	private Function translateFunction(FlowGraphEnv env, BasicBlock bb, Node sn, SourceLocation srcLoc, int resultReg) {
		...
		maxReg = oldMaxTmp;
		//System.out.println("Last SRC LOC: "+lastSrcLoc.getLineNumber());
		return f;
	}
...
//		int line=0;
		...
		case Token.CALL: { // [new] [fn-name]([arg1],...,[argn])
			...
			//System.out.println("*******"+firstChild.getQualifiedName());
...
//				line= prop.getCharno();
			...
			CallNode cn;
			if (complexChildType)
				// cn = new CallNode(snType == Token.NEW, resultReg, objReg, propReg, propStr, rhsRegs, srcLoc); //ORIGINAL TAJS
				cn = new CallNode(firstChild.getQualifiedName(), snType == Token.NEW, resultReg, objReg, propReg, propStr, rhsRegs, srcLoc);
//				cn = new CallNode(snType == Token.NEW, resultReg, objReg, propReg, propStr, rhsRegs, srcLoc ,line);
			else
				//cn = new CallNode(,snType == Token.NEW, false, resultReg, objReg, lhsReg, rhsRegs, srcLoc); //ORIGINAL TAJS
				cn = new CallNode(firstChild.getQualifiedName(), snType == Token.NEW, false, resultReg, objReg, lhsReg, rhsRegs, srcLoc);
//				cn = new CallNode(snType == Token.NEW, false, resultReg, objReg, lhsReg, rhsRegs, srcLoc, line);
		...
		case Token.ARRAYLIT: {
			...
			currBB = newSuccessorBasicBlock(env, currBB);
			//addNodeToBlock(env, currBB, new CallNode(true, true, resultReg, AbstractNode.NO_VALUE, arrReg, argRegs, srcLoc), resultReg, statement); // ORIGINAL TAJS
			addNodeToBlock(env, currBB, new CallNode(sn.getQualifiedName(), true, true, resultReg, AbstractNode.NO_VALUE, arrReg, argRegs, srcLoc), resultReg, statement);
//			addNodeToBlock(env, currBB, new CallNode(true, true, resultReg, AbstractNode.NO_VALUE, arrReg, argRegs, srcLoc, line), resultReg, statement);
		...
		case Token.REGEXP: { // /[regexp]/[flags]
			...
			//addNodeToBlock(callBlock, new CallNode(true, false, resultReg, AbstractNode.NO_VALUE, reReg, args, srcLoc), statement); // ORIGINAL TAJS
			addNodeToBlock(callBlock, new CallNode(sn.getQualifiedName(), true, false, resultReg, AbstractNode.NO_VALUE, reReg, args, srcLoc), statement);
//			addNodeToBlock(callBlock, new CallNode(true, false, resultReg, AbstractNode.NO_VALUE, reReg, args, srcLoc, line), statement);
